...
std::unique_ptr<MainFuncDef> Parser::parseMainFuncDef()
{
    ...
    consume(TokenType::KEYWORD_INT, "int");
    advance();
...
{
    size_t pos = current_ + ahead;
    if (pos < tokens_.size())
    {
        Token token = tokens_[pos];
        return token;
    }
    return Token(TokenType::ERROR, "越界", 0, 0);
}
...
void Parser::advance()
{
    ...
    {
        token_.tokenType_ = TokenType::END_OF_FILE;
    }
}
